...
load("XXXXXX/NHANES_DietPattern_19992004.rda")
demo_df <- readRDS("XXXXXX/demo_df.rds")

# 1. Obtain dietary data ------------------------------------------------------------------
load("XXXXXX/NHANES_19992000.rda")
load("XXXXXX/NHANES_20012002.rda")
load("XXXXXX/NHANES_20032004.rda")
load("XXXXXX/NHANES_20052006.rda")
load("XXXXXX/NHANES_20072008.rda")
load("XXXXXX/NHANES_20092010.rda")
load("XXXXXX/NHANES_20112012.rda")
load("XXXXXX/NHANES_20132014.rda")
load("XXXXXX/NHANES_20152016.rda")
load("XXXXXX/NHANES_20172020.Rda")
...
NHANES_20052018_dietaryindex_d1d2 <-
  reduce(list(
  ...
  function(df1, df2)
    full_join(df1, df2))
NHANES_20052018_dietaryindex_d1d2 %>% nrow  # 49576
...
NHANES_19992018 = rbind(
  NHANES_19992004[, intersect(names(NHANES_19992004), names(NHANES_20052018_dietaryindex_d1d2))],
  NHANES_20052018_dietaryindex_d1d2[, intersect(names(NHANES_19992004), names(NHANES_20052018_dietaryindex_d1d2))]
)
  ...
  for (X_variable in diet_indexx) {
    formula <- as.formula(paste(
      ...
      paste('factor(', X_variable, ') + Age + Sex + Smoke + factor(Ethnicity) + PA_2018 +',
            'factor(Education) +  factor(PIR) + Average_cal +',
            'BMI + Diabetes + Hypertension + cancer', collapse = "")
    ))
    model <- svyglm(formula, design = design)
    info <- extract_info(model)
    results[[paste(Y_variable, paste(X_variable, collapse = "+"), sep = "_")]] <- info
